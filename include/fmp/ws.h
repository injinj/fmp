#ifndef __rai_fmp__ws_h__
#define __rai_fmp__ws_h__
...
struct HttpClientCB {
  ...
  virtual void on_ws_msg( WSMsg &msg ) noexcept;
};

struct HttpClient : public ds::SSL_Connection {
  HttpClientCB * cb;
  ...
  HttpClient( kv::EvPoll &p )
    : ds::SSL_Connection( p, 0 ), cb( 0 ), ws_mask( 0 ), ws_bytes_sent( 0 ),
      is_websock( false ) {
    this->rand.init();
  }
  ...
  void send_ws_pong( WSMsg &ping ) noexcept;
